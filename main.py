...
def criar_pdf_bonito(df, col_nome, col_custo, nome_pdf, caminho_grafico):
    ...
    # Mostra até 10 registros para manter o design limpo
    ...
    # --- ÁREA INFERIOR (Gráfico à esquerda e Assinatura à direita) ---
    pdf.ln(10)
    y_pos = pdf.get_y()
    
    if os.path.exists(caminho_grafico):
        # Gráfico ocupa o lado esquerdo
        pdf.image(caminho_grafico, x=15, y=y_pos, w=140)
    
    # Assinatura posicionada à direita do gráfico
    pdf.set_xy(180, y_pos + 20)
    pdf.set_draw_color(0, 0, 0)
    pdf.line(180, y_pos + 25, 270, y_pos + 25) # Linha da assinatura
    
    pdf.set_xy(180, y_pos + 27)
    ...
    pdf.cell(90, 10, f"Luanda, {datetime.now().strftime('%d/%m/%Y')}", ln=True, align='C')
...
def enviar_email(pdf_nome):
    ...
    msg = MIMEMultipart()
    msg['Subject'] = f"🚀 RELATORIO LOGISTICA PREMIUM: {datetime.now().strftime('%d/%m/%Y')}"
    msg.attach(MIMEText("Olá Laurindo, segue o relatório com design corrigido e página única.", 'plain'))
...
def executar():
    ...
    try:
        ...
        # Gráfico com proporções melhores
        ...
        plt.tight_layout()
        plt.savefig('grafico_elegante.png')
        plt.close()
        
        nome_pdf = "Relatorio_Laurindo_Premium.pdf"
        criar_pdf_bonito(df, 'Endereço', col_custo, nome_pdf, 'grafico_elegante.png')
