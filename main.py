...
# Classe para formatar o cabeçalho oficial
...
def criar_pdf_final_oficial(df, col_nome, col_custo, nome_pdf, caminho_grafico):
    pdf = PDF_Logistica('L', 'mm', 'A4') # Formato Paisagem
    pdf.add_page()
    ...
    # Definição das colunas conforme o print de sucesso
    ...
    for _, row in df.iterrows():
        ...
        # Cores para o status
        if status_raw.lower() in ['ok', 'concluido', 'concluído']:
    ...
    pdf.set_text_color(200, 0, 0) # Total em Vermelho
    pdf.cell(30, 10, f"{total_geral:,.2f}", border=1, fill=True, align='C')
    pdf.set_text_color(0, 0, 0)
    pdf.cell(15, 10, "100%", border=1, fill=True, align='C')
    pdf.cell(182, 10, " Kwanzas (Relatorio Oficial Luanda)", border=1, ln=True, fill=True)
    ...
    data_gerada = datetime.now().strftime('%d/%m/%Y %H:%M')
    pdf.cell(90, 5, f"Relatorio gerado em Luanda: {data_gerada}", ln=True, align='C')
...
def enviar_email(pdf_nome):
    ...
    msg['From'] = meu_email
    msg['To'] = destinatario
    msg.attach(MIMEText("Segue em anexo o relatorio oficial detalhado em formato horizontal.", 'plain'))
...
def executar():
    ...
    try:
        ...
        # Gráfico Verde conforme o print
        ...
        criar_pdf_final_oficial(df, col_nome, col_custo, nome_pdf, 'grafico_oficial.png')
