...
def criar_pdf_executivo(df, col_nome, col_custo, nome_pdf):
    ...
    # --- LOGO (COR ALTERADA PARA AZUL ROYAL) ---
    pdf.set_fill_color(0, 102, 204) # Azul Royal Vibrante
    pdf.rect(15, 15, 15, 15, 'F')  
    pdf.set_text_color(255, 255, 255) 
    pdf.set_font("Arial", 'B', 12)
    pdf.text(18, 25, "LL") 

    # CABEÇALHO (NOMES DA EMPRESA EM AZUL)
    pdf.set_xy(35, 15)
    pdf.set_font("Arial", 'B', 18)
    pdf.set_text_color(0, 102, 204) # Azul Royal
    pdf.cell(100, 10, "LAURINDO LOGISTICA & SERVICOS", ln=True)
    
    pdf.set_font("Arial", 'I', 9)
    pdf.set_text_color(100, 100, 100) # Mudei o laranja para um Cinza Executivo
    pdf.set_xy(35, 22)
    ...
    # --- TABELA (COR ALTERADA PARA AZUL ROYAL) ---
    pdf.set_font("Arial", 'B', 12)
    pdf.set_fill_color(0, 102, 204) # Cabeçalho da tabela Azul
    pdf.set_text_color(255, 255, 255) 
...
def enviar_email(img_nome, pdf_nome):
    ...
    msg.attach(MIMEText("Ola Laurindo, segue o relatorio com as novas cores azuis.", 'plain'))
...
def verificar_e_enviar_tudo():
    ...
    try:
        ...
        if not caros.empty:
            ...
            # --- COR DO GRÁFICO ALTERADA PARA AZUL ---
            plt.bar(caros['Endereço'].str[:15], caros[col_custo], color='royalblue')
            plt.title('Analise de Custos de Transporte')
            plt.savefig('grafico_final.png')
            
            criar_pdf_executivo(caros, 'Endereço', col_custo, "Relatorio_Cores_Novas.pdf")
            enviar_email('grafico_final.png', "Relatorio_Cores_Novas.pdf")
            print("Sucesso: Cores atualizadas!")
