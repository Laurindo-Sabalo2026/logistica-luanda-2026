...
def criar_pdf_final(df, col_nome, col_custo, nome_pdf, caminho_grafico):
    ...
    largura_destino = 80 if tem_status else 115
    
    pdf.set_font("Arial", 'B', 11)
    pdf.set_fill_color(0, 102, 204)
    pdf.set_text_color(255, 255, 255)
    
    pdf.cell(largura_destino, 10, " Destino", border=1, fill=True)
    ...
    for _, row in df.iterrows():
        pdf.cell(largura_destino, 10, f" {str(row[col_nome])[:35]}", border=1)
    ...
    pdf.cell(0, 8, f"Gerado em Luanda - Data: {data_hoje}", ln=True, align='C')
...
def enviar_email(pdf_nome):
    ...
    msg.attach(MIMEText("Relatorio oficial Laurindo Logistica atualizado.", 'plain'))
...
def executar():
    ...
    try:
        ...
        if not caros.empty:
            ...
            criar_pdf_final(caros, 'Endereço',
